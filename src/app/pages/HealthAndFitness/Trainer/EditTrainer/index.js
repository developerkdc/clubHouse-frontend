...
const EditTrainer = () => {
  ...
  const getTrainerDetails = async () => {
    try {
      const res = await Axios.get(`/health_fitness/trainer/list?id=${id}`);
  ...
  const handleTrainerEdit = async (data) => {
    ...
    try {
      await Axios.patch(`/health_fitness/trainer/edit/${id}`, formData);
